...
const AdminProjectDetail = () => {
  ...
  const overallProgress = useMemo(() => {
    if (project?.progress !== undefined && project?.progress !== null) {
      return Number.isFinite(Number(project.progress))
                    ...
                    {project.disputes.map((dispute) => {
                      const statusStyle = (() => {
                        ...
                        if (s === "RESOLVED") {
                          return {
                            container:
                              "bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-800 hover:bg-emerald-100/50",
                        ...
                        return {
                          container:
                            "bg-red-50 dark:bg-red-900/10 border-red-100 dark:border-red-800 hover:bg-red-100/50",
                      ...
                      return (
                        ...
                        >
                          ...
                          <p
                            className={`text-sm line-clamp-2 ${statusStyle.muted}`}
                          >
                            {dispute.description}
                          </p>
                        </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {derivedPhases.map((phase) => (
                  <div
                    key={phase.id}
                    className="flex items-start gap-3 pb-3 border-b border-border/60 last:border-0 rounded p-2"
                  >
            ...
            <Card>
              ...
              <CardContent>
                <Accordion type="single" collapsible className="w-full">
                  {derivedPhases.map((phase) => {
                    const phaseTasks = derivedTasks.filter(
                      (t) => t.phase === phase.id
                    );
                    return (
              ...
              <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
                {project.conversation ? (
                  <>
                    ...
                    project.conversation.messages.length > 0 ? (
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="pt-2 text-xs text-muted-foreground border-t mt-2">
                  Joined{" "}
                  {format(
                    new Date(project.owner?.createdAt || new Date()),
                    "MMM yyyy"
                  )}
                </div>
              </CardContent>
