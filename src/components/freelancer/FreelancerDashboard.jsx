...
export const DashboardContent = ({ roleOverride }) => {
  ...
  useEffect(() => {
    const loadMetrics = async () => {
      ...
      try {
        ...
        console.log("DEBUG: Accepted Proposals:", accepted.map(p => ({
            id: p.id,
            amount: p.amount, 
            projectSpent: p.project?.spent, 
            projectStatus: p.project?.status
        })));
        console.log("DEBUG: totalReceived:", totalReceived, "totalPending:", totalPending);
  ...
  const handleNotificationClick = (notification) => {
    ...
    if (notification.type === "chat" && notification.data) {
      ...
      navigate(projectId ? `/freelancer/messages?projectId=${projectId}` : "/freelancer/messages");
    ...
    <div className="flex-1 flex flex-col relative h-full overflow-hidden bg-zinc-50 dark:bg-black transition-colors duration-300">
      ...
      <header className="sticky top-0 z-40 px-6 py-4 flex items-center justify-between transition-colors duration-300 border-b border-border/40 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md supports-[backdrop-filter]:bg-white/60 dark:supports-[backdrop-filter]:bg-zinc-900/60">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={toggleSidebar} className="lg:hidden text-muted-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Profile */}
          <div className="text-right hidden sm:block cursor-pointer" onClick={() => navigate("/freelancer/profile")}>
            <p className="text-sm font-bold text-foreground leading-none">{sessionUser?.fullName || "Freelancer"}</p>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-8">
              ...
              <div className="flex justify-between items-end">
                <div>
                  <h1 className="text-3xl md:text-4xl font-black tracking-tighter text-foreground mb-2">Dashboard</h1>
                  <p className="text-muted-foreground font-medium">Overview of your freelance activity.</p>
                </div>
                <div className="hidden sm:flex gap-2">
                  <Badge variant="outline" className="px-3 py-1 rounded-full bg-primary/10 text-primary-dark border-primary/20 gap-1.5 font-bold">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="bg-card p-6 rounded-2xl border border-border shadow-sm hover:shadow-md hover:border-primary/30 transition-all relative overflow-hidden group">
                  ...
                  <div className="relative z-10">
                    <p className="text-muted-foreground text-sm font-bold mb-1 uppercase tracking-wide">Total Earnings</p>
                    <h3 className="text-3xl font-black tracking-tight text-foreground">{formatCurrency(metrics.earnings)}</h3>
                    <p className="text-xs text-green-500 mt-2 flex items-center font-bold">
                      <TrendingUp className="h-3 w-3 mr-1" /> Estimated (70% share)
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-card p-6 rounded-2xl border border-border shadow-sm hover:shadow-md hover:border-blue-500/30 transition-all relative overflow-hidden group">
                  ...
                  <div className="relative z-10">
                    <p className="text-muted-foreground text-sm font-bold mb-1 uppercase tracking-wide">Active Jobs</p>
                    <h3 className="text-3xl font-black tracking-tight text-foreground">{metrics.activeProjects}</h3>
                    <div className="flex items-center gap-2 mt-2">
                      <div className="flex-1 h-1.5 bg-secondary rounded-full overflow-hidden">
                        <div className="h-full bg-blue-500 rounded-full" style={{ width: `${Math.min((metrics.activeProjects / 5) * 100, 100)}%` }}></div>
                      </div>
                      <span className="text-xs font-medium text-blue-500">Utilization</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-card p-6 rounded-2xl border border-border shadow-sm hover:shadow-md hover:border-purple-500/30 transition-all relative overflow-hidden group">
                  ...
                  <div className="relative z-10">
                    <p className="text-muted-foreground text-sm font-bold mb-1 uppercase tracking-wide">Pending Proposals</p>
                    <h3 className="text-3xl font-black tracking-tight text-foreground">{metrics.proposalsReceived}</h3>
                    <p className="text-xs text-purple-500 mt-2 flex items-center font-bold">
                      <Clock className="h-3 w-3 mr-1" /> Awaiting client response
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-bold text-foreground">Active Jobs Overview</h3>
                  <Button variant="link" className="text-primary hover:text-primary/80 h-auto p-0 font-semibold" onClick={() => navigate("/freelancer/project")}>
                        ...
                        metrics.acceptedProposals.map((proposal) => (
                          <tr key={proposal.id} className="group hover:bg-secondary/30 transition-colors">
                            ...
                            <td className="px-6 py-4 text-right">
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary" onClick={() => navigate(`/freelancer/project/${proposal.project?.id}`)}>
            ...
            {/* Dynamic Upcoming Meeting Widget (Moved to TOP for visibility) */}
              ...
              <div className="bg-primary/5 dark:bg-card rounded-2xl p-6 border border-primary/10 dark:border-border shadow-lg relative overflow-hidden">
                ...
                <h3 className="font-bold text-lg mb-4 flex items-center gap-2 text-foreground relative z-10">
                  ...
                  <Badge className="bg-primary text-black hover:bg-primary/90 ml-auto font-bold">{metrics.proposalsReceived}</Badge>
                </h3>
                
                <div className="flex flex-col gap-3 relative z-10">
                  {metrics.pendingProposals.length === 0 ? (
                    <div className="text-sm text-muted-foreground italic">No pending proposals.</div>
                  ) : (
                    metrics.pendingProposals.slice(0, 3).map(p => (
                      <div key={p.id} className="bg-background/80 backdrop-blur-sm p-4 rounded-xl border border-border hover:border-primary/30 transition-all">
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex-1 min-w-0 pr-2">
                            <p className="text-sm font-bold text-foreground truncate">{p.project?.title}</p>
                            <p className="text-xs text-muted-foreground">Client: {p.project?.owner?.fullName}</p>
                          </div>
                          <span className="text-xs font-bold text-foreground whitespace-nowrap">{formatCurrency(p.amount)}</span>
                        </div>
                        <div className="flex gap-2 mt-2">
                          <Button size="sm" variant="outline" className="w-full text-xs h-7 font-bold" onClick={() => navigate("/freelancer/proposals")}>
                            View Details
                          </Button>
                        </div>
                      </div>
              ...
              <div className="bg-card rounded-2xl border border-border p-6 shadow-sm">
                <h3 className="font-bold text-lg text-foreground mb-6">Earnings Goal</h3>
                <div className="flex items-center gap-6">
                  ...
                  <div className="relative w-28 h-28 flex-shrink-0">
                    <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                      ...
                      <circle
                        ...
                        strokeDashoffset={263.89 - (263.89 * (metrics.earnings > 0 ? (metrics.receivedEarnings / metrics.earnings) : 0))}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-2xl font-black text-foreground">
                        {metrics.earnings > 0 ? Math.round((metrics.receivedEarnings / metrics.earnings) * 100) : 0}%
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-4">
                    <div>
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Received Payment</p>
                      <p className="text-xl font-black text-foreground leading-none">{formatCurrency(metrics.receivedEarnings || 0)}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Pending Payment</p>
                      <p className="text-xl font-black text-foreground leading-none">{formatCurrency(metrics.pendingEarnings || 0)}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-card rounded-2xl border border-border p-6 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-lg text-foreground">Active Clients</h3>
                  <Button variant="link" className="text-primary hover:text-primary/80 h-auto p-0 font-semibold text-sm" onClick={() => navigate("/freelancer/messages")}>
                    View All
                  </Button>
                </div>
                <ul className="flex flex-col gap-4">
                  {metrics.acceptedProposals.length === 0 ? (
                    <div className="text-sm text-muted-foreground italic">No active clients yet.</div>
                  ) : (
                    ...
                    metrics.acceptedProposals.slice(0, 5).map((proposal) => (
                      <li key={proposal.id} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-foreground truncate">{proposal.project?.owner?.fullName || "Client"}</p>
                          <p className="text-xs text-muted-foreground truncate">{proposal.project?.title}</p>
                        </div>
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary rounded-full transition-colors" onClick={() => navigate(`/freelancer/messages?projectId=${proposal.projectId}`)}>
